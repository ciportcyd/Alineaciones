...
export default function App() {
  ...
  const exportRef = useRef(null); // contenedor que exportaremos
  ...
  async function exportPNG() {
    ...
    const date = new Date().toISOString().split("T")[0]; // ej: 2025-10-12
    const filename = `alineacion_${date}.png`;
  ...
  return (
    <div className="w-screen min-h-screen bg-neutral-900 text-white flex flex-col gap-2 p-2">
      ...
      <div className="flex justify-end mb-2">
        <button
          onClick={exportPNG}
          className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 rounded-lg font-semibold text-white"
        >
          📸 Exportar Alineación
        </button>
      </div>

      {/* 🖼️ Contenedor a exportar */}
      <div ref={exportRef}>
            ...
            {onField.map(p => (
              ...
              >
                <img
                  src={p.img}
                  alt={p.name}
                  className="w-24 sm:w-24 md:w-28 h-auto shadow-md pointer-events-none select-none"
                  draggable={false}
...
function Header({ title, count }) {
  return (
    <div className="flex items-center justify-between mb-2">
      ...
      <span className="text-xs bg-neutral-700 rounded px-2 py-0.5">{count}</span>
    </div>
  );
}
      ...
      {players.map(p => (
        ...
        >
          <img
            src={p.img}
            alt={p.name}
            className="w-16 h-auto pointer-events-none select-none"
            draggable={false}
            onError={(e)=>{ e.currentTarget.style.opacity = 0.3; }}
          />
          <div className="text-[10px] leading-none mt-1 opacity-80 truncate w-full text-center px-1">
            {p.name}
          </div>
        </div>
      ))}
